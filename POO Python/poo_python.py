...
class ContaCorrente(Conta):
    def __init__(self, numero, cliente, limite=500, limite_saques=3):
        ...
        self._saques_do_dia = 0

    def sacar(self, valor):
        hoje = datetime.now().date()

        if self._ultimo_dia != hoje:
            self._ultimo_dia = hoje
            self._saques_do_dia = 0

        excedeu_limite = valor > self.limite
        excedeu_saques = self._saques_do_dia >= self.limite_saques
...
if __name__ == "__main__":
    cliente = PessoaFisica(nome="Marcelo", data_nascimento="1990-01-01", cpf="12345678900", endereco="Rua Exemplo")
